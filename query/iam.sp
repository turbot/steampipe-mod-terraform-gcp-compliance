...
query "iam_project_use_basic_role" {
  sql = <<-EOQ
    select
      ...
      name || case
        when (arguments ->> 'role') like any (array ['roles/owner', 'roles/editor', 'roles/viewer']) then ' use basic role'
...
query "iam_service_account_no_admin_priviledge" {
  sql = <<-EOQ
    select
      ...
      name || case
        when (arguments ->> 'member') like '%.iam.gserviceaccount.com$' and (arguments ->> 'role') like any (array ['roles/Admin', 'roles/admin', 'roles/owner', 'roles/editor']) then ' has admin priviledge'
        else ' does not have admin priviledge'
      end || '.' reason
      ${local.common_dimensions_sql}
    from
      terraform_resource
...
query "iam_project_use_default_service_role" {
  sql = <<-EOQ
    select
      ...
      name || case
        when ((arguments ->> 'members') like any (array ['%@developer.gserviceaccount.com', '%@appspot.gserviceaccount.com']) or (arguments ->> 'member') like '%@developer.gserviceaccount.com' or (arguments ->> 'member') like '%@appspot.gserviceaccount.com') then ' use default service account role'
